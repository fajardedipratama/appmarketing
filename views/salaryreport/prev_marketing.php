...
$karyawan = Karyawan::find()->where(['departemen'=>$dept_id])->andWhere(['<=','tanggal_masuk',$model->akhir_cutoff])->orderBy('nama')->all();
...
<?php if($show['tgl_resign']==NULL || $show['tgl_resign']>=$model->awal_cutoff): ?>
    <?php
        $result_po=PurchaseOrder::find()->where(['sales'=>$show['id']])->andWhere(['between','tgl_kirim',$model->awal_cutoff,$model->akhir_cutoff])->andWhere(['status'=>['Terkirim','Terbayar-Selesai']])->sum('volume');
        $cod_a=PurchaseOrder::find()->where(['sales'=>$show['id']])->andWhere(['between','tgl_kirim',$model->awal_cutoff,$model->akhir_cutoff])->andWhere(['termin'=>['Cash On Delivery','Cash Before Delivery']])->andWhere(['<=','range_paid',0])->sum('volume');
        $cod_b=PurchaseOrder::find()->where(['sales'=>$show['id']])->andWhere(['between','tgl_kirim',$model->awal_cutoff,$model->akhir_cutoff])->andWhere(['termin'=>['Cash On Delivery']])->andWhere(['between','range_paid',1,2])->sum('volume');
        $pot_absen = SalaryAdditional::find()->where(['karyawan_id'=>$show['id'],'komponen_id'=>4])->andWhere(['between','tanggal',$model->awal_cutoff,$model->akhir_cutoff])->sum('nilai');
    ?>
        <tr>
            <td><?= $i++ ?></td>
            <td><?= $show['nama'] ?></td>
